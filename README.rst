...
#. If needed, override existing config settings by creating a new file ``src/config.local``. You can override all settings found in ``src/config``. If you need to override the path to the base image used for building FullPageOS, set ``BASE_ZIP_IMG`` to the exact file. By default, the most recent Raspberry Pi OS file matching ``*-raspios*.{zip,7z,xz}`` found in ``src/image`` will be used, so keep only the image for the architecture you are building there.
